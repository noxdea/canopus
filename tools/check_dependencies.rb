...
expected = %w[alhena alkaid antares denebola editorconfig kochab megrez porrima prism rouge sadr spica tarazed thuban timeout unicode-display_width zaniah]
...
Dir.mktmpdir("#{entry}-install-") do |directory|
  ...
  environment = ENV.each_key.grep(/\ABUNDLE/).to_h { |key| [key, nil] }.merge(
    ...
    "MEGREZ_PATH" => nil, "SADR_PATH" => nil, "TARAZED_PATH" => nil
  )
  {"ALKAID_PATH" => "alkaid", "ANTARES_PATH" => "antares", "MEGREZ_PATH" => "megrez", "SADR_PATH" => "sadr",
